...
compose_dependency_impl = $(foreach file,$(call uniq,$(foreach glob_file,$(call glob,raw/$(1)/$(2)@*.!(txt)),$(basename $(glob_file)))),$(call img_candidate,$(1),$(file)))
...
LATEXMK = latexmk.pl -recorder -use-make -lualatex -deps -f \
	-e '@cus_dep_list = ();' \
	-e 'show_cus_dep();'
...
latex/main.pdf: preamble
	...
	@$(LATEXMK) -pdf -deps-out=$(DEPS_DIR)/$(BASENAME_REL).d -outdir=latex $(FILE_REL_TEX)
...
latex/img/%.tex: $$(call img_candidate,img,%) $$(wildcard raw/img/%.txt) script/to_latex/cleanup.py script/to_latex/img.py
	$(call echolog,"Formatting $*...")
	script/to_latex/img.py $* img $(suffix $(call img_candidate,img,$*))
	
latex/img/%.tex: $$(call compose_dependency,img,%) $$(wildcard raw/img/%.txt) script/to_latex/cleanup.py script/to_latex/compose_img.py
...
latex/graph/%.tex: $$(call img_candidate,graph,%) $$(wildcard raw/graph/%.txt) script/to_latex/cleanup.py script/to_latex/img.py
	$(call echolog,"Formatting $*...")
	script/to_latex/img.py $* graph $(suffix $(call img_candidate,graph,$*))
	
latex/graph/%.tex: $$(call compose_dependency,graph,%) $$(wildcard raw/graph/%.txt) script/to_latex/cleanup.py script/to_latex/compose_img.py
	$(call echolog,"Composing $*...")
